...
public class RecoveryPlanTest {
    ...
    @Test public void manyPlaylistAlternativesAreCapped(){RecoveryPlan p=new RecoveryPlan();List<Channel.Stream> sources=new ArrayList<>();for(int i=0;i<10;i++)sources.add(stream("https://a/"+i));for(int i=0;i<4;i++)assertNotNull(p.next(sources));assertNull(p.next(sources));assertEquals(4,p.attempts());}
}
